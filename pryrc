...
def prompt_setup(options = {})
  ...
  target_self_proc = Proc.new do |tgt_self|
    "#{tsop}#{colorize_object(Pry.view_clip(tgt_self), options[:obj_color])}#{tscp}"
  end

  nest_lvl_proc = Proc.new do |nest_lvl|
